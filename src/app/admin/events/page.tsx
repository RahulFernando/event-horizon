"use client";
import React, { useContext } from "react";
import useSWR from "swr";
import useSWRMutation from "swr/mutation";
import { Typography, Box, Chip } from "@mui/material";
import AdminLayout from "@/app/components/admin-layout";
import { Event } from "@prisma/client";
import { AuthContext } from "@/app/contexts/auth/auth-context";
import {
  DataGrid,
  GridColDef,
  GridRenderCellParams,
  GridRowParams,
} from "@mui/x-data-grid";
import dayjs from "dayjs";
import useDialog from "@/app/hooks/use-dialog";
import Dialog from "@/app/components/dialog";
import EventDetail from "./components/event-detail";
import SnackBar from "@/app/components/snack-bar";
import { SnackbarContext } from "@/app/contexts/snackbar/snackbar-context";
import { ActionKind } from "@/app/contexts/snackbar/snackbar.types";

async function fetchEvents(url: string, token: string) {
  const response = await fetch(url, {
    headers: {
      Authorization: `Bearer ${token}`,
      "Content-Type": "application/json",
    },
  });

  if (!response.ok) {
    const error = await response.json();
    throw new Error(error?.message || "Failed to update vendor");
  }

  return (await response.json()) as Event[];
}

async function updateStatus(
  url: string,
  token: string,
  { args }: { args: { enabled: boolean } }
) {
  const response = await fetch(url, {
    method: "PATCH",
    headers: {
      Authorization: `Bearer ${token}`,
      "Content-Type": "application/json",
    },
    body: JSON.stringify(args),
  });

  if (!response.ok) {
    const error = await response.json();
    throw new Error(error?.message || "Something went wrong");
  }

  return await response.json();
}

const defaultColumns: GridColDef[] = [
  {
    field: "title",
    headerName: "Title",
    flex: 1,
  },
  {
    field: "date_time",
    headerName: "Date Time",
    flex: 1,
    valueGetter: (params, row) => dayjs(row.date_time).format("LLL"),
    renderCell: (params: GridRenderCellParams<Event>) =>
      dayjs(params.row.date_time).format("LLL"),
  },
  {
    field: "duration",
    headerName: "Duration",
    flex: 1,
    renderCell: (params: GridRenderCellParams<Event>) =>
      params.row.duration ? params.row.duration + " " + "hrs" : "----",
  },
  {
    field: "venue",
    headerName: "Venue",
    flex: 1,
  },
  {
    field: "enabled",
    headerName: "Enabled",
    flex: 1,
    renderCell: (params: GridRenderCellParams<Event>) => (
      <Chip
        size="small"
        label={params.row.enabled ? "Enabled" : "Disabled"}
        color={params.row.enabled ? "success" : "error"}
      />
    ),
  },
];

const AdminEventPage = () => {
  const { token } = useContext(AuthContext);
  const { snackbarToggle } = useContext(SnackbarContext);

  const { open, info, clickCloseHandler, clickOpenHandler } = useDialog();

  const { data: events = [], mutate } = useSWR("/api/events", (url: string) =>
    fetchEvents(url, token as string)
  );

  const { isMutating, trigger: changeStatus } = useSWRMutation(
    info?.data && `/api/events/${info.data.id}`,
    (url: string, { arg }: { arg: { enabled: boolean } }) =>
      updateStatus(url, token as string, { args: arg }),
    {
      onSuccess: () => {
        mutate();
        snackbarToggle(ActionKind.OPEN, {
          open: true,
          message: "Status changed successfully",
          severity: "success",
        });
        clickCloseHandler();
      },
      onError: (err) => {
        snackbarToggle(ActionKind.OPEN, {
          open: true,
          message: err.message,
          severity: "error",
        });
      },
    }
  );

  const buttonLabel = info && info.data.enabled ? "Disable" : "Enable";

  const rowClickHandler = (param: GridRowParams<Event>) =>
    clickOpenHandler({ data: param.row });

  const disableHandler = () => changeStatus({ enabled: !info?.data.enabled });

  return (
    <>
      <SnackBar />
      <AdminLayout>
        <Typography variant="h4">Events</Typography>
        <Box sx={{ height: 500, width: "100%", mt: 2 }}>
          <DataGrid
            rows={events}
            columns={defaultColumns}
            onRowClick={rowClickHandler}
          />
        </Box>
      </AdminLayout>
      <Dialog
        open={open}
        title={`${info?.data?.title}`}
        content={<EventDetail id={info?.data.id} />}
        confirmButtonLabel={buttonLabel}
        disableSubmitButton={isMutating}
        onConfirm={disableHandler}
        onClose={clickCloseHandler}
      />
    </>
  );
};

export default AdminEventPage;
